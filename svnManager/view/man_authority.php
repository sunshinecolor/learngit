		...
		<div class="con_title">
            ...
            <div class="search">
                <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="get">
                    <input type="text" name="keyword" id="" placeholder="请输入用户名或姓名"/>
                    <input type="submit" value="搜索" />
                </form>
            </div>
		</div>
        <div class='opreat left auth_opreat'>
            <button class='all_btn'>个人操作权限</button>
            <button class='lost_btn all_btn proa_all_btn'>用户组操作权限</button>
        </div>
		<div class='user_info left' style="margin:20px auto;">
			<div class="view_bor view_usera_bor">
				<div class="view_all">
						<table border="1" class="user-mes">
						<tr>
							<th>用户名</th>
                            <th>姓名</th>
							<th>所属用户组</th>
						...
						<tr>
							...
							<td><?php if(!empty($v['groups'])){
                                    foreach($v['groups'] as $va){
                                        echo $va['group_name'].'</br>';
                                    }
                                }else{
                                    echo '无';
                                }?></td>
							<td>
								<?php if(!empty($v['auth_user'])){
									foreach($v['auth_user'] as $item){
                                        echo $item.'</br>';
                                    }
								}else{
									echo '无';
                            ...
                            <td>
                                <?php if(!empty($v['auth_group'])){
                                    foreach($v['auth_group'] as $item){
                                        echo $item.'</br>';
                                    }
                                }else{
                                    echo '无';
							...
							<td><button class="edit_btn" value="<?php echo $v['user_id'];?>" attr-name="user" user-name="<?php echo $v['realname']?>" edit-id="<?php echo $v['user_id']?>">编辑</button></td>
						</tr>
						<?php }?>
						</table>
                    <?php echo $link;?>
...
<script type="text/javascript">
	$(function(){
		$('.lost_btn').click(function(){
			window.location.href='man_authority_group.php';
		});
